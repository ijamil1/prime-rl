...
- **Gradient diagnostic replay:** when `trainer.experimental.gradient_diagnostic.mode = "replay"`, the local RL launcher starts only the trainer process. It skips inference, orchestrator, trainer weight broadcast, and shared W&B mode so the trainer can replay recorded microbatches from `source_dir` without waiting for absent peer processes. Single-run LoRA replay uses a trainer-local synthetic `run_default` at index 0, so optimizer setup does not wait for an orchestrator-written `orch.toml`. Replay first pads each recorded tensor microbatch's sequence length to the current CP world size with non-loss-contributing tokens, then pads uneven DP distributions with zero-loss dummy microbatches before strided rank assignment.
